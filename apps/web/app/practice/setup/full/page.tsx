...
const examSteps = [
  ...
  { part: "Part 2", title: "Cue Card", titleZh: "话题卡", time: "3-4 min", detail: "1 minute preparation and long-turn answer." },
  ...
  return (
    <AcademicShell activePath="/practice" userName={user?.display_name || user?.email} userRole={user?.role}>
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_390px]">
          ...
          <div className="relative grid gap-3">
            ...
            {examSteps.map((step, index) => (
              <article key={step.part} className="relative grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 sm:grid-cols-[46px_1fr_auto] sm:items-center">
                <span className="z-10 flex h-10 w-10 items-center justify-center rounded-lg border border-[#D4AF37]/35 bg-white text-sm font-semibold text-[#8A6F1D]">
                  {index + 1}
                </span>
                <div>
        ...
        <Panel>
          <SectionHeading icon={ShieldCheck} label="Exam Settings" labelZh="进入前确认" />
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <Mic2 className="h-4 w-4 text-[#3A7CA5]" />
                  <div>
                    <p className="text-sm font-semibold text-slate-900">Microphone pre-check</p>
                    <p className="text-xs text-slate-500">浏览器麦克风检查</p>
                  </div>
                </div>
  ...
  return (
    <label className="flex min-h-14 cursor-pointer items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2">
      ...
      <input
        ...
        className="h-5 w-5 cursor-pointer rounded border-slate-300 text-[#D4AF37] accent-[#D4AF37]"
      />
    </label>
  );
}
