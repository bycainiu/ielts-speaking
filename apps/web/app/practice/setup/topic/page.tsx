  ...
  return (
    <AcademicShell activePath="/practice" userName={user?.display_name || user?.email} userRole={user?.role}>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
        <Panel>
          <SectionHeading icon={BookOpenCheck} label="Topic Catalog" labelZh="主题检索" />
                ...
                return (
                  <button
                    ...
                    className={`min-h-[138px] cursor-pointer rounded-lg border p-4 text-left transition-colors ${
                      selected ? "border-[#D4AF37] bg-[#FFF8DF]" : "border-slate-200 bg-white hover:border-[#D4AF37]/50 hover:bg-[#FFFDF7]"
                    }`}
                  >
            ...
            {selectedTopic ? (
              <div className="grid gap-4">
                <div>
                  ...
                  <h2 className="mt-3 font-serif text-3xl text-[#0B132B]">{selectedTopic.name}</h2>
                      ...
                      <button
                        ...
                        className={`min-h-11 cursor-pointer rounded-lg border text-sm font-semibold transition-colors ${
                          selectedParts.includes(part)
                            ? "border-[#D4AF37] bg-[#0B132B] text-white"
                  ...
                  {partCounts.map((item) => (
                    <div key={item.part} className="grid grid-cols-[56px_1fr_36px] items-center gap-2 text-sm">
                      ...
                      <span className="h-2 overflow-hidden rounded-full bg-white">
                        <span className="block h-full rounded-full bg-[#3A7CA5]" style={{ width: `${Math.min(100, item.count * 18)}%` }} />
                      </span>
                ...
                <label className="flex min-h-14 cursor-pointer items-center justify-between gap-3 rounded-lg border border-[#D4AF37]/25 bg-white px-3 py-2">
