...
export function useAgentRunStream(runId: string, options?: { enabled?: boolean }) {
  ...
  const appendEvents = useCallback((incoming: AgentStreamEvent[]) => {
    ...
    async (afterSeq: number) => {
      ...
      try {
        const replayResponse = await authenticatedFetch(
          `/api/agent-harness/agent/runs/${encodeURIComponent(runId)}/events?after_seq=${afterSeq}`,
          {
            cache: "no-store",
        ...
        const replayEvents = (await replayResponse.json()) as AgentStreamEvent[];
        ...
        const streamResponse = await authenticatedFetch(
          `/api/agent-harness/agent/runs/${encodeURIComponent(runId)}/stream?after_seq=${streamAfterSeq}`,
          {
            cache: "no-store",
            signal: controller.signal,
          },
        ...
        const message = err instanceof Error ? err.message : "Agent stream disconnected.";
        setError(message);
