...
class MiMoChatClient:
    ...
    async def _stream_json(self, url: str, payload: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
        async def open_stream() -> httpx.Response:
            # 不能用 client.stream() 上下文管理器后丢弃引用：它基于 asynccontextmanager，
            # 生成器一旦被 GC 会触发 response.aclose()，导致流在读取中途被掐断（ReadError）。
        ...
        try:
            ...
            while True:
                # 逐行读取时手动推进迭代器，把中途的网络错误统一包装成 MiMoError，
                # 避免 httpx.ReadError 等异常穿透到工作流层。
                try:
                    line = await anext(iterator)
                except StopAsyncIteration:
                    break
                except httpx.TimeoutException as exc:
                    raise MiMoTimeoutError(str(exc) or "MiMo stream timed out") from exc
                except httpx.HTTPError as exc:
                    raise MiMoAPIError(
                        str(exc) or "MiMo stream interrupted",
                        code="network_error",
                        retryable=True,
                    ) from exc
...
def build_openai_payload(
    ...
    *,
    ...
    if stream:
        # 让 OpenAI 兼容端在最后一个 chunk 返回真实 token 用量。
...
def parse_openai_stream_chunk(data: dict[str, Any]) -> ChatStreamChunk:
    ...
    if not choices:
        # include_usage 的收尾 chunk 没有 choices，但携带最终 token 用量。
        return ChatStreamChunk(usage=parse_openai_usage(data.get("usage")), raw=data)
    choice = choices[0]
    delta = choice.get("delta") or {}
    usage = parse_openai_usage(data.get("usage"))
    return ChatStreamChunk(
        delta=str(delta.get("content") or ""),
        reasoning_delta=str(delta.get("reasoning") or delta.get("reasoning_content") or delta.get("thinking") or ""),
        finish_reason=choice.get("finish_reason"),
...
def parse_anthropic_stream_chunk(data: dict[str, Any]) -> ChatStreamChunk:
    ...
    if event_type == "content_block_delta":
        ...
        if delta.get("type") == "thinking_delta":
            return ChatStreamChunk(reasoning_delta=str(delta.get("thinking") or ""), raw=data)
...
def parse_openai_usage(value: Any) -> ChatUsage:
...
def parse_anthropic_usage(value: Any) -> ChatUsage:
    if not isinstance(value, dict):
        return ChatUsage()
    return ChatUsage(
        input_tokens=value.get("input_tokens"),
        output_tokens=value.get("output_tokens"),
        total_tokens=sum_tokens(value.get("input_tokens"), value.get("output_tokens")),
